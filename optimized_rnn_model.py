...
class CryptoPredictor:
    ...
    def fit_and_evaluate(self, X, y, feature_names=None):
        ...
        for name, model_builder in model_configs.items():
            print(f"\nTesting {name} model:")
            cv_results = self.walk_forward_validation(X_train, y_train, model_builder, n_splits=3)
        ...
        final_model = best_model[1](X_train.shape[1:])
...
def main():
    ...
    X_mac = pd.read_csv('X_MACRO.csv')
    
    # X_df = X_df.merge(X_mac, left_index=True, right_index=True, how='')
    ...
    # combined_df['SP500_Return_20d', 'SP500_Daily_Return' ,'SP500_Rolling_Vol_20d', 'SP500_Daily_Return_Lag1'] = X_mac['SP500_Return_20d' ,'SP500_Daily_Return', 'SP500_Rolling_Vol_20d', 'SP500_Daily_Return_Lag1']
    ...
    # Clip percentreturn to [-0.5, 0.5] range
    y_clipped = np.clip(y, -0.5, 0.5)
    # y_clipped = np.clip(y, -1.0, 1.0)

    # Verify data quality after clipping
    print(f"Target range after clipping: [{y_clipped.min():.4f}, {y_clipped.max():.4f}]")
    print(f"NaN values in target after clipping: {np.isnan(y_clipped).sum()}")
    print(f"Number of values at -0.5: {(y_clipped == -0.5).sum()}")
    print(f"Number of values at 0.5: {(y_clipped == 0.5).sum()}")
    ...
    joblib.dump(predictor, 'crypto_predictor_optimized51.pkl')
    joblib.dump(results, 'crypto_results_optimized51.pkl')
...
# class CryptoPredictor:
#     def __init__(self, sequence_length=30, validation_method='walk_forward'):
#         self.sequence_length = sequence_length  # Increased to 30
#         self.validation_method = validation_method
...
#         for period in [5, 10, 20, 50]:  # Added 50-day momentum
#             df[f'momentum_{period}d'] = df['Close'].pct_change(period)
#             df[f'volume_momentum_{period}d'] = df['Volume'].pct_change(period)
...
#         # Data quality check
#         print("Data Quality Check:")
#         print(f"Shape: {X.shape}, NaN values: {np.isnan(X).sum()}")

#         # Remove NaN values
#         self.pipeline = make_pipeline(KNNImputer(n_neighbors=5), RobustScaler())
#         X = self.pipeline.fit_transform(X)
#         y = y[~np.isnan(y)]
        
#         # Feature selection (increased to 15 features)
#         top_features = mutual_info_regression(X, y, random_state=42)
#         top_features = np.argsort(top_features)[-15:]
#         X_selected = X[:, top_features]
        
#         # Scale features
#         self.pipeline = make_pipeline(KNNImputer(n_neighbors=5), RobustScaler())
#         X_scaled = self.pipeline.fit_transform(X_selected)
...
#         self.results = {'test_rmse': test_rmse, 'test_mae': test_mae, 'r2_score': r2_score}
#         return self.results
...
#     predictor = CryptoPredictor(sequence_length=30)
#     combined_df = X_df.copy()
